...
func TestBool(t *testing.T) {
	...
	var b bool
	validateDecoding(t, bools, reflect.ValueOf(&b))
}
...
func validateDecoding(t *testing.T, tests map[string]interface{}, result reflect.Value) {
	for inputStr, expected := range tests {
		inputBytes, _ := hex.DecodeString(inputStr)
		d := decoder{inputBytes, 0}
		_, err := d.decode(0, result)
		if err != nil {
			t.Error(err)
		}
		if !reflect.DeepEqual(result.Elem().Interface(), expected) {
